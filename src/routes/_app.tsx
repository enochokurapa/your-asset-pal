...
const nav: NavItem[] = [
  ...
  { to: "/saas-admin", label: "SaaS Admin", icon: ShieldCheck, saasAdminOnly: true },
...
function AppLayout() {
  ...
  const visibleNav = nav.filter((n) => {
    if (n.saasAdminOnly && !isSaasAdmin) return false;
    if (n.tenantAdminOnly && !isTenantAdmin && !isSaasAdmin) return false;
  ...
  const blockTrialReportExport = (e: React.MouseEvent<HTMLElement>) => {
    if (!pathname.startsWith("/reports") || canExportReports) return;
    ...
    <div className="flex min-h-screen w-full bg-background">
      ...
      >
        <div className="flex h-16 items-center justify-between border-b border-sidebar-border px-5">
          <div className="flex items-center gap-2">
            <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-sidebar-primary text-sidebar-primary-foreground"><Boxes className="h-5 w-5" /></div>
            <div><p className="text-sm font-semibold leading-none">AssetFlow</p><p className="mt-0.5 text-[10px] uppercase tracking-wider text-sidebar-foreground/60">Asset Manager</p></div>
          </div>
        ...
        <div className="border-t border-sidebar-border p-3">
          <div className="mb-2 rounded-lg bg-sidebar-accent/50 px-3 py-2">
            ...
            <p className="mt-0.5 text-[10px] uppercase tracking-wider text-sidebar-foreground/60">
              {isSaasAdmin ? "SaaS admin" : isTenantAdmin ? "Tenant admin" : roles.length ? roles.join(" · ") : "member"}
            </p>
          </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="flex-1 px-4 py-6 md:px-8 md:py-8" onClickCapture={blockTrialReportExport}>
          {pathname.startsWith("/reports") && !canExportReports && subscriptionStatus === "trial" && (
